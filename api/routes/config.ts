...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    // Verify Alchemy API key configuration
    const alchemyApiKey = process.env.ALCHEMY_API_KEY_POLYGON || process.env.ALCHEMY_API_KEY_MAINNET;
    
    if (!alchemyApiKey) {
      console.log('❌ ALCHEMY_API_KEY_POLYGON or ALCHEMY_API_KEY_MAINNET not configured');
      return res.status(500).json({
        error: 'ALCHEMY_API_KEY_POLYGON or ALCHEMY_API_KEY_MAINNET not configured',
        configured: false
      });
    }

    console.log('✅ ALCHEMY_API_KEY_POLYGON / ALCHEMY_API_KEY_MAINNET verified');
